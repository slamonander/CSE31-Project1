...
int check_surrounding_indexes(char **arr, char *word, int index, int row, int column, int size) {
    int **currentGrid = (int**) malloc(bSize*sizeof(int*)); // Attempting to allocate memory for this array, but it's not properly initialized, meaning 
    // there's segmentation faults
    ...
    if (*(*(arr + row) + column) != *(word + index)) { // If the current character doesn't match, exit
        return 0;
    } // This fucked up the code by completely terminating the function and returning 0, which makes the word not found at all
                ...
                if (i < bSize && j < bSize) {
                    if (!(i == row && j == column)) {
                        if (*(*(arr + i) + j) == *(word + index)) {
                            ...
                            *(*(currentGrid + i) + j) = *(*(arr + i) + j) * 10 + (index + 1); // 2d array wasn't properly allocated and initialized, leading to undefined behavior
                            return 1;
                            }
                        }
                    }
